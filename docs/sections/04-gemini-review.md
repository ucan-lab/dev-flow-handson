...
### ローカルで AI レビューすればよくない?

ローカルレビューも有効。ただし、それは「push 前のセルフチェック」であって、PR 上の共有レビューとは役割が違う。

| 観点             | ローカル AI レビュー            | Gemini Code Assist on GitHub      |
| ---------------- | ------------------------------- | --------------------------------- |
| 実行タイミング   | 自分で実行する必要がある        | PR open / コマンドで自動実行      |
| 見える範囲       | 実行した本人の手元に閉じがち    | PR コメントとしてチーム全員に残る |
| レビュー漏れ     | 忘れる / 面倒で飛ばす余地がある | PR フローに組み込める             |
| 指摘への対応履歴 | チャットログに散らばりやすい    | GitHub のスレッドで追跡できる     |
| モデルの多様性   | 実装と同じ AI を使いがち        | Claude 実装 + Gemini レビュー     |

結論: ローカル AI レビューはセルフチェック、Gemini Code Assist はチームに見える一次レビュー。競合ではなく、置く場所が違う。
