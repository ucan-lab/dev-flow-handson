...
```markdown
- [ ] #123 ログイン画面のバリデーション修正 @alice
- [ ] #128 商品一覧の N+1 解消 @bob
- [ ] #131 通知メールテンプレ更新 @carol
```
...
### 2. リポジトリ直下に `.git-pr-release` (PR テンプレ) を追加
...
- **対象 PR の絞り込み**: ラベルやユーザーで除外したいときは `.git-pr-release` テンプレ内で `pull_requests` を Ruby のフィルタで絞る
