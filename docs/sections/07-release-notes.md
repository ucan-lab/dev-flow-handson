# Section 7: リリースノート作成自動化

使うツール: [Release Drafter](https://github.com/release-drafter/release-drafter)

## Why — なぜやるか

Section 6 の git-pr-release は **「次のリリースに何が乗るか」を可視化する** ところまでが守備範囲。
...
Section 6 で `develop → main` 運用に切り替えたので、Release Drafter の設定ファイルも同じフローに乗せる。
